...
didCreateSessionDescription:(RTCSessionDescription *)origSdp
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        ...
        //NSLog(@"send message pearconnection create ---------------");
        [self sendMessage:data];
        
        NSDictionary *userInfo = [NSDictionary dictionaryWithObject:error2.description?error2.description:data forKey:SEND_ERROR];
        [[NSNotificationCenter defaultCenter] postNotificationName:SEND_ERROR object:nil userInfo:userInfo];
    });
}
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection
    ...
    //NSLog(@"send message pearconnection signal ---------------");
...
//          candidate.sdp);
    NSDictionary *json =
    @{ @"type" : @"candidate",
       @"label" : [NSNumber numberWithInt:(int)candidate.sdpMLineIndex],
       @"id" : candidate.sdpMid,
